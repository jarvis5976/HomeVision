...
export function PowerFlowCard() {
   ...
   * Hub: (40, 50)
   * Solar: Center (40, 15), Bottom Edge (40, 18)
   * Grid: Center (10, 50), Right Edge (13, 50)
   * Battery: Center (40, 85), Top Edge (40, 82)
   * Maison: Center (70, 50), Left Edge (66.5, 50), Top Edge (70, 46.5), Bottom Edge (70, 53.5)
   * Borne: Center (70, 20), Bottom Edge (70, 23)
   * Cumulus: Center (70, 80), Top Edge (70, 77)
   */
      ...
      <CardContent className="pt-4 pb-10">
        <div className="relative w-full max-w-[800px] mx-auto aspect-[16/10] flex items-center justify-center">
          ...
          <svg viewBox="0 0 100 100" className="absolute inset-0 w-full h-full overflow-visible">
            ...
            {/* Trajectoires de flux rectilignes optimisées */}
            
            {/* 1. Solaire -> Hub */}
            <path id="path-solar" d="M 40 18 L 40 50" className="stroke-muted/20" strokeWidth="0.8" fill="none" />
            {flows.solar > 20 && (
              <circle r="0.8" fill="#fbbf24" filter="url(#glow-path)">
                <animateMotion dur={`${getDuration(flows.solar)}s`} repeatCount="indefinite" path="M 40 18 L 40 50" />
              </circle>
            )}

            {/* 2. Réseau -> Hub */}
            <path id="path-grid" d="M 13 50 L 40 50" className="stroke-muted/20" strokeWidth="0.8" fill="none" />
            {Math.abs(flows.grid) > 20 && (
              <circle r="0.8" fill={flows.isExporting ? "hsl(var(--primary))" : "#f43f5e"} filter="url(#glow-path)">
                <animateMotion 
                  ...
                  path={flows.isExporting ? "M 40 50 L 13 50" : "M 13 50 L 40 50"} 
                />
              </circle>
            )}

            {/* 3. Hub -> Batterie */}
            <path id="path-battery" d="M 40 50 L 40 82" className="stroke-muted/20" strokeWidth="0.8" fill="none" />
            {Math.abs(flows.battery) > 20 && (
              <circle r="0.8" fill="#10b981" filter="url(#glow-path)">
                <animateMotion 
                  dur={`${getDuration(flows.battery)}s`} 
                  repeatCount="indefinite" 
                  path={flows.isBatteryCharging ? "M 40 50 L 40 82" : "M 40 82 L 40 50"} 
                />
              </circle>
            )}

            {/* 4. Hub -> Maison */}
            <path id="path-house" d="M 40 50 L 66.5 50" className="stroke-muted/20" strokeWidth="0.8" fill="none" />
            {flows.house > 20 && (
              <circle r="0.8" fill="hsl(var(--primary))" filter="url(#glow-path)">
                <animateMotion dur={`${getDuration(flows.house)}s`} repeatCount="indefinite" path="M 40 50 L 66.5 50" />
              </circle>
            )}

            {/* 5. Maison -> Borne (Verticale) */}
            <path id="path-borne" d="M 70 46.5 L 70 23" className="stroke-muted/20" strokeWidth="0.8" fill="none" />
            {flows.borneWatts > 20 && (
              <circle r="0.8" fill="#3b82f6" filter="url(#glow-path)">
                <animateMotion dur={`${getDuration(flows.borneWatts)}s`} repeatCount="indefinite" path="M 70 46.5 L 70 23" />
              </circle>
            )}

            {/* 6. Maison -> Cumulus (Verticale) */}
            <path id="path-cumulus" d="M 70 53.5 L 70 77" className="stroke-muted/20" strokeWidth="0.8" fill="none" />
            {flows.cumulusWatts > 20 && (
              <circle r="0.8" fill="#f97316" filter="url(#glow-path)">
                <animateMotion dur={`${getDuration(flows.cumulusWatts)}s`} repeatCount="indefinite" path="M 70 53.5 L 70 77" />
              </circle>
            )}
          </svg>
          ...
          {/* Borne Recharge (Haut Droite - Au dessus de la maison) */}
          <div className="absolute left-[70%] top-[20%] -translate-x-1/2 -translate-y-1/2 flex flex-col items-center gap-1 group">
          ...
          {/* Cumulus (Bas Droite - En dessous de la maison) */}
          <div className="absolute left-[70%] top-[80%] -translate-x-1/2 -translate-y-1/2 flex flex-col items-center gap-1 group">
